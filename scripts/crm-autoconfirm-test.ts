...
async function main(): Promise<void> {
  ...
  // ─── 归属判定 ──────────────────────────────────────────────────────────────
  {
    ...
    ok('A1 自动确认后核销状态仍 pending（认领≠核销）', String(a.reconciliation_status ?? '') === 'pending')
    ok('A1 认领未核销前 creditedTotal=0', crmDbService.creditedTotal(Number(d.payload?.contract_id)) === 0)
    ok('A1 显式核销后 creditedTotal=7600', crmDbService.reconcileAllocation(aid).ok && crmDbService.creditedTotal(Number(d.payload?.contract_id)) === 7600)
    ok('A1 核销幂等（重复核销不重复计数）', crmDbService.reconcileAllocation(aid).ok === true && crmDbService.creditedTotal(Number(d.payload?.contract_id)) === 7600)
