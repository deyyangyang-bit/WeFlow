...
try {
  ...
  check('只新增 caddy 服务，保留 postgres / central', Boolean(services.postgres && services.central && services.caddy))
  ...
  check('Caddy 不挂载 Central .env 或 secret', !asArray(caddy.volumes).some((volume) => /\.env|secret|postgres_password/i.test(JSON.stringify(volume))))
  ...
  check('proxy.env 与 Caddy 运行目录被 Docker 忽略', dockerignore.includes('**/proxy.env') && dockerignore.includes('**/caddy-data') && dockerignore.includes('**/caddy-config'))
