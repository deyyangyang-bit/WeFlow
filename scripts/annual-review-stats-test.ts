 ...
 *     非法 generatedAt / 下一年零点只进下一年度 / asOf 时刻本身不计入
...
function main(): void {
  ...
  {
    // WCDB 秒区间换算：毫秒 → 秒向下取整；all_time 下界 0
  ...
  // ══ C A3 ═════════════════════════════════════════════════════════════════
  {
    ...
    // WCDB 失败 → last_contact_at 秒→毫秒回退
    const fbFacts: AnnualReviewFacts = {
      ...emptyFacts(),
      accounts: [
        account(1, 'wx_a', Math.floor(T(2025, 3, 1, 8) / 1000)),   // 2025 年内 → 活跃
        account(2, 'wx_b', Math.floor(J2025 / 1000)),              // 左边界（秒整）→ 活跃
        account(3, 'wx_c', Math.floor(J2026 / 1000) - 1),          // asOf 前最后 1 秒 → 活跃
        account(4, 'wx_d', Math.floor(J2026 / 1000)),              // asOf 整秒 → 不计
        account(5, 'wx_e', Math.floor(T(2024, 6, 1) / 1000)),      // 往年 → 不计
        account(6, 'wx_manual', Math.floor(T(2025, 3, 1) / 1000)), // 排除名单 → 不计
        account(7, 'wx_f', null)                                   // 无回填 → 不计
      ]
    }
    const fb = summary(P2025, fbFacts, { messageStats: { ok: false, sessions: {} }, exclusions: { manualSessions: ['wx_manual'] } })
    ...
    const fbNull = summary(P2025, fbFacts, { messageStats: null, exclusions: { manualSessions: ['wx_manual'] } })
    ...
    const dead = summary(P2025, { ...emptyFacts(), accounts: [account(1, 'wx_a', null)] }, { messageStats: { ok: false, sessions: {} } })
    ...
    ok('C4b 空客户档案 + WCDB 失败 → 0（partial 保守态）', summary(P2025, emptyFacts(), { messageStats: { ok: false, sessions: {} } }).customerActive.value === 0)
    // 非法消息数值 → 按 0 处理 + 告警降级
