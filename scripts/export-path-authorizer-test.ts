 ...
 *
 * 说明（S8）：本脚本原另含「旧 /annual-report 图片导出载荷策略 + annualReportImageExport
 * 生产函数」两段用例；该产品链路已整体下线（模块同批删除），故这两段随实现一并移除，
 * 不再保留针对已删代码的断言。authorizer 本体的覆盖不受影响。
...
const saveRoots = (roots: Array<{ path: string; realPath: string; grantedAt: number }>): void => {
  writeFileSync(storePath, JSON.stringify(roots))
}
