...
- **下一刀**：**P0-3.3 SalesContextStrip**（消费 currentView.judgments 替代 customerGet+intentHistory+actionSuggest 自拼；suggest 现场生成属用户主动触发，评估保留）→ P0-3.4 今日行动 + 见解收件箱（历史 JSON 依赖，最后迁移）→ P0-3 收口：**静态护栏扫描**（src/ 无 UI 直读 customer_profile.stage / insight_record / follow_up_task.analysis / generateInsight / generateActionAnalysis 现场调用）。
