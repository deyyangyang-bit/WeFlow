...
- **刀3 `057f8aa` feat(sales) action analysis 三调用点统一落 judgment**（盘点刀3 契约）：
  ...
  - **三调用点统一接入**：预热（salesActionEngine，写 analysis JSON 后追加落库）/ suggest（main.ts，用户主动生成）/ 客户 360（crmIpcHandlers，合成 item id=0 + triggerType='customer_profile'）。替代「预热写 analysis JSON + on-demand 丢弃」。
...
- **下一刀**：**P0-2C 收口**（C 三刀全量回归终验 + P0-2 收口），随后进入 P0-2 UI（AI 当前判断卡：消费 `judgmentCurrentAll` + P0-2B 证据回查）。
