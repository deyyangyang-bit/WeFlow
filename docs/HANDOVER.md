...
- **② 真实 DB 只读盘点**（脚本 `scripts/p0-2-real-db-audit.ts`，sql.js 内存加载纯只读）：**200 客户**，canonical stage 分布 contacted=76/quoted=67/won=26/lost=21/negotiating=9/**unknown=1**（六档齐全，存量脏值几乎清零）；intent_tag_log 1131 条，**今日写入 1/1 带 messageKey ✓**（P0-1 透传真实生效，evidence=客户原话）；follow_up_task 2524 条；**customer_judgment 表尚未出现在真实库**（应用最后一次启动早于 C.1 提交，建表随启动自动完成 → 判断覆盖率 0 基线属部署时序非缺陷，应用下次重启后重复跑脚本即可量化）。
