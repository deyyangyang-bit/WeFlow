...
- **下一刀**：**P0-3 收口**（用户拍板：先别进下一阶段）——① 全仓静态护栏扫描：src/ 无 UI 直读 `customer_judgment` / `insight_record` 作为当前判断 / `follow_up_task.analysis` 作为当前判断 / UI 现场 `generateInsight`·`generateActionAnalysis`；✅ 消费统一走 `sales:customer:currentView` ② 真实库运行态验收 → P0-3 CLOSED 声明（HANDOVER + AGENTS.md）。
