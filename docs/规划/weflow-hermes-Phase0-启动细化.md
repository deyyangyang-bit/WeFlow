...
### D5 · 接口契约（docs/API-CONTRACT.md，**9/2 拍板收缩版**）—— ✅ 已完成（2026-09-02，GLM 实现 + Kimi 验收：现有 113 通道从代码梳理 + 9 个 Phase 0/1 新端点规范级 + NAS/MCP 占位规范，未超纲）

| 层 | 深度 | 理由 |
|---|---|---|
| IPC 层 | 端点级完整 | Phase 0/1 立即要用（分配/绑定/审计查询） |
| 本机 HTTP 只读 | 端点级（延续 HTTP-API.md） | 已有基础 |
| NAS 内网 API | 只定规范（认证/版本/幂等/错误码）+ Phase 3a 端点占位清单 | Phase 3 才开工，现在写细节 = 猜需求 |
