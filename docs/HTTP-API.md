...
> **边界（2026-09-14）**：本文只描述**销售本机的 HTTP API**（默认 `http://127.0.0.1:5031`，读聊天/会话/联系人与本地 SSE 推送）。
> **中央主机内网 API 是另一个服务**（不同主机、不同鉴权、不同网络边界），端点契约见 `docs/API-CONTRACT.md` §3，
> **不写入本文档**——两者命名、认证与网络边界分别维护，禁止混为同一接口。
