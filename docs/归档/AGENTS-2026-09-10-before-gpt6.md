...
- **标注指引 `docs/评测集标注指引.md`**（1 页给非技术主管）：三档判定标准 / 操作流程 / AI 预标注用法（先自己判再对 AI 答案）/ 目标量（W8 前 ≥100 条，无商机对照 ≥30%）
